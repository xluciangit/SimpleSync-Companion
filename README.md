...
Android companion app for [SimpleSync Server](https://github.com/xluciangit/SimpleSyncServer). Pick which folders to watch, set how often to scan, and it quietly uploads new and changed files in the background. Includes a dedicated WhatsApp backup mode.
...
- WhatsApp backup — backs up your WhatsApp folder to the server on a schedule; files are stored separately from regular sync folders and only re-uploaded if changed
...
Tap the gear icon on a folder card to change the scan interval (default 60 minutes), toggle hidden file syncing, or remove the folder. Removing it only deletes the local config — files already on the server are not touched.

---

## WhatsApp backup

When enabled, the app backs up your WhatsApp folder to the server on a configurable schedule. It works differently from regular sync folders:

- Files are stored under a dedicated `WhatsApp` folder on the server, separate from your regular sync folders
- Each file is checked by path and hash — only new or changed files are uploaded
- The backup runs in the background via WorkManager, same as regular sync

To enable it, go to **Settings** and turn on WhatsApp Backup, then set the backup interval. The app will find your WhatsApp folder automatically.

On the server side, the WhatsApp folder is treated separately — when an admin deletes a user, the WhatsApp folder can be kept even if other files are removed.
...
8. Failure → job gets one automatic retry after all other pending jobs finish; if it fails again it's marked Failed permanently and can be manually retried
