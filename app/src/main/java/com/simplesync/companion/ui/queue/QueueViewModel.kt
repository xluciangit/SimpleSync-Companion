...
class QueueViewModel(app: Application) : AndroidViewModel(app) {
        ...
        fun update() {
            ...
            value = when (f) {
                FilterType.UPLOADING -> jobs.filter {
                    it.status == JobStatus.HASHING || it.status == JobStatus.UPLOADING
    ...
    init {
        // auto-clear completed/skipped jobs 10 min after finishing
                ...
                .collectLatest { jobs ->
                    ...
                    jobs.filter {
                        ...
                        if (scheduledClears.add(job.id)) {
                            val remaining = (600_000L - (now - job.completedAt!!)).coerceAtLeast(0L)
                            launch {
                                delay(remaining)
    ...
    fun pauseUploads() = viewModelScope.launch {
        ...
        com.simplesync.companion.worker.WhatsAppBackupWorker.cancel(getApplication())
        com.simplesync.companion.worker.UploadWorker.cancelAll(getApplication())
    }
...
enum class FilterType { UPLOADING, PENDING, COMPLETED, FAILED, CANCELLED }
