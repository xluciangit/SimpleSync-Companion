...
class SyncRepository(private val context: Context) {
    ...
    suspend fun markUploaded(job: UploadJob, sha256Hash: String? = null) {
        // Try to read current lastModified from the document; fall back to 0 if unavailable.
        val lastMod = try {
            DocumentFile.fromSingleUri(context, Uri.parse(job.fileUriString))?.lastModified() ?: 0L
