...
class UploadWorker(ctx: Context, params: WorkerParameters) : CoroutineWorker(ctx, params) {
    ...
    private suspend fun doWorkLocked(): Result = withContext(Dispatchers.IO) {
    ...
    private suspend fun processJob(
        ...
        return try {
            ...
            val sha256 = hashFromUri(uri, atomicHashBytes)
            hashProgressJob.cancel()

            val isWaJob = job.folderConfigId == SyncRepository.WA_FOLDER_ID
            val hashExists = if (isWaJob) {
                val (exists, changed) = repo.checkWhatsAppFile(job.relativePath, sha256)
                exists && !changed
            } else {
                checkHashOnServer(serverUrl, apiKey, sha256, job.remoteFolderName)
            }
            if (hashExists) {
                repo.updateJobStatus(job.id, JobStatus.SKIPPED, prog = job.fileSize)
                if (isWaJob) {
                    val doc = androidx.documentfile.provider.DocumentFile.fromSingleUri(applicationContext, uri)
                    if (doc != null) repo.saveWaCache(job.relativePath, doc.lastModified(), job.fileSize, sha256)
                } else {
            ...
            val uploadEndpoint = if (isWaJob) "$uploadBase/api/whatsapp/upload" else "$uploadBase/api/upload"
    ...
    private fun hashFromUri(uri: Uri, progress: AtomicLong): String {
